...
public final class ClientCalls {
  ...
  public static <ReqT, RespT> RespT blockingUnaryCall(
    ...
    System.out.println(
        Thread.currentThread().getName() + " created ThreadlessExecutor " + executor.hashCode());
    ...
    try {
      ...
      while (!responseFuture.isDone()) {
        try {
          System.out.println(Thread.currentThread().getName() + " Wait and drain");
      ...
      if (interrupt) {
        System.out.println(Thread.currentThread().getName() + " Setup interrupted flag");
   ...
   * @throws StatusRuntimeException if {@code get} throws an {@link ExecutionException} or an {@link
  ...
  private static StatusRuntimeException toStatusRuntimeException(Throwable t) {
    Throwable cause = Preconditions.checkNotNull(t, "t");
    while (cause != null) {
    ...
    private Object waitForNext() {
      ...
      try {
        if (threadless == null) {
          ...
          System.out.println(Thread.currentThread().getName() + " got next " + next);
          return next;
        }
      } finally {
        if (interrupt) {
          System.out.println(Thread.currentThread().getName() + " return interrupted flag");
          Thread.currentThread().interrupt();
        }
      }
    }
    ...
    public void waitAndDrain() throws InterruptedException {
      ...
      do {
        try {
          ...
          runnable.run();
          System.out.println(
              Thread.currentThread().getName() + " finish executing runnable " + runnable);
        } catch (Throwable t) {
          System.out.println(Thread.currentThread().getName() + " runnable threw an exception");
