...
public class AsyncActivityTest {
      ...
      TestWorkflowRule.newBuilder()
          ...
          .setWorkerFactoryOptions(
              WorkerFactoryOptions.newBuilder()
                  ...
                  .validateAndBuildWithDefaults())
          .setWorkflowTypes(
              WorkflowImplWithFunctionalReferenceCall.class,
              WorkflowImplWithActivityInLambdaCall.class)
  ...
  public static class WorkflowImplWithFunctionalReferenceCall implements TestWorkflow1 {
    ...
    public String workflow(String input) {
      return Async.function(activity::activity, input).get();
    }
  }

  public static class WorkflowImplWithActivityInLambdaCall implements TestWorkflow2 {
    private final TestActivity activity =
        Workflow.newActivityStub(
            TestActivity.class,
            ActivityOptions.newBuilder()
   ...
   */
  @Test
  public void testAsyncActivityByReference() {
    ...
    assertEquals("StartWorkflow:TestWorkflow1", workflowStartSpan.operationName());
    ...
    assertEquals("RunWorkflow:TestWorkflow1", workflowRunSpan.operationName());
  ...
  @Test
  public void testAsyncActivityByLambda() {
    MockSpan span = mockTracer.buildSpan("ClientFunction").start();

    WorkflowClient client = testWorkflowRule.getWorkflowClient();
    try (Scope scope = mockTracer.scopeManager().activate(span)) {
      TestWorkflow2 workflow =
          client.newWorkflowStub(
              TestWorkflow2.class,
              WorkflowOptions.newBuilder()
                  .setTaskQueue(testWorkflowRule.getTaskQueue())
    ...
    MockSpan workflowStartSpan = spansHelper.getByParentSpan(clientSpan).get(0);
    assertEquals(clientSpan.context().spanId(), workflowStartSpan.parentId());
    assertEquals("StartWorkflow:TestWorkflow2", workflowStartSpan.operationName());

    MockSpan workflowRunSpan = spansHelper.getByParentSpan(workflowStartSpan).get(0);
    assertEquals(workflowStartSpan.context().spanId(), workflowRunSpan.parentId());
    assertEquals("RunWorkflow:TestWorkflow2", workflowRunSpan.operationName());
